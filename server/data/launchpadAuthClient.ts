...
const timeoutSpec = config.apis.launchpadAuth.timeout
const launchpadAuthUrl = config.apis.launchpadAuth.url

function getSystemClientTokenFromLaunchpadAuth(username?: string): Promise<superagent.Response> {
  const clientToken = generateOauthClientToken(
    config.apis.launchpadAuth.systemClientId,
    config.apis.launchpadAuth.systemClientSecret,
  )
...
export default class LaunchpadAuthClient {
  ...
  async getSystemClientToken(username?: string): Promise<string> {
    ...
    const newToken = await getSystemClientTokenFromLaunchpadAuth(username)
