...
export default class NomisMappingService {
  ...
  async nomisToDpsLocation(locationId: number, prisonerId: string, agencyId: string) {
    logger.info(`Mapping NOMIS location ID to DPS location ID: ${locationId}`, { prisonerId, agencyId })
    ...
    try {
      ...
      logger.error(`Error mapping NOMIS location ID to DPS location ID: ${locationId}`, { prisonerId, agencyId, error })
